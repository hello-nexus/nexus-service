...
/// <summary>Raw rendered frames before the wire-budget downsample pass.</summary>
internal readonly record struct RawAnimation(byte[] Frames, int FrameCount, double IntervalMs);

/// <summary>
/// Frame math shared by every pre-rendered wireless animation (Strimer cables
/// and fan chains): buffer layout, colour blending and the size-budget
/// downsample pass. Geometry (lane count, LEDs per lane) is caller-supplied so
/// the same math covers a cable's lanes and a fan chain's per-fan ring.
/// </summary>
internal static class Slv3WirelessEffectMath
{
    internal static byte[] AllocateFrames(int frameCount, int lanes, int ledsPerLane) =>
        new byte[frameCount * lanes * ledsPerLane * 3];

    internal static void SetLed(byte[] buf, int frame, int lanes, int ledsPerLane, int lane, int led, RgbColor c)
    {
    ...
    internal static int Pos(int index, int count, int direction) => direction != 0 ? count - 1 - index : index;

    internal static byte ScaleChannel(byte value, double factor) => (byte)Math.Clamp(value * factor, 0, 255);

    internal static RgbColor Scale(RgbColor c, double factor) =>
        new(ScaleChannel(c.R, factor), ScaleChannel(c.G, factor), ScaleChannel(c.B, factor));

    internal static RgbColor Lerp(RgbColor a, RgbColor b, double t)
    {
    ...
    internal static RgbColor Hue(double hue01)
    {
    ...
    internal static double Breath(double t) => (Math.Sin((t - 0.25) * 2 * Math.PI) + 1.0) / 2.0;

    /// <summary>Maps a repeating phase to a 0..1 travel value that bounces back at each end instead of wrapping.</summary>
    internal static double BounceTravel(double phase)
    {
    ...
    internal static void PaintMarker(
        byte[] buf, int frame, int lanes, int ledsPerLane, int lane, double centerPos, double trailLen, RgbColor color, int direction)
    {
        for (var led = 0; led < ledsPerLane; led++)
        {
            ...
            SetLed(buf, frame, lanes, ledsPerLane, lane, led, Scale(color, intensity));
        }
    }

    internal static RawAnimation Loop(int frameCount, int lanes, int ledsPerLane, double intervalMs, Action<byte[], int> paint)
    {
    ...
    /// </summary>
    internal static Slv3StrimerAnimation Finalize(RawAnimation raw, int lanes, int ledsPerLane)
    {
        var stride = lanes * ledsPerLane * 3;
    ...
    internal static bool TryCompress(ReadOnlySpan<byte> data, out int compressedLength)
    {
        try
        {
