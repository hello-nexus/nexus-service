...
public static class PanelRoutes
{
    public static void MapPanelEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/panel/devices/{id}", (string id, PanelDevicePatch body, HttpContext ctx, PanelDeviceRegistry registry, MultiplexHub hub, TokenService tokens, Nexus.Service.Panel.Streams.StreamedPanelCoordinator streams, IConfigStore store) =>
        {
            ...
            if (body.Layout is not null)
                store.Update(Nexus.Service.Deck.DeckInstanceGc.Run);
