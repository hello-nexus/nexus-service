...
# Bundled marketplace apps (nexus-apps, copied to <staging>/apps/ by the
# csproj Content include). The AppRegistry scans <BaseDirectory>/apps/, which
# for the .app is Contents/MacOS/apps/ - without this copy the app ships zero
# marketplace apps even though the publish tree has them.
[ -d "$STAGING/apps" ] && { rm -rf "$APP/Contents/MacOS/apps"; cp -R "$STAGING/apps" "$APP/Contents/MacOS/"; }
