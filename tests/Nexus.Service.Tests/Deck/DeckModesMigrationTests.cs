...
public sealed class DeckInstanceGcTests
{
    [Fact]
    public void WidgetInstance_NoLongerInAnyLayout_IsRemoved()
    {
        var doc = new NexusSettings();
        doc.StreamDeck.Instances["widget:gone"] = new DeckInstance { Mode = "fixed" };

        DeckInstanceGc.Run(doc);

        Assert.False(doc.StreamDeck.Instances.ContainsKey("widget:gone"));
    }

    [Fact]
    public void WidgetInstance_StillInAPanelDeviceLayout_Survives()
    {
        var doc = new NexusSettings();
        doc.StreamDeck.Instances["widget:w1"] = new DeckInstance { Mode = "fixed" };
        doc.PanelDevices["dev1"] = new PanelDeviceRecord
        {
            Id = "dev1",
            Layout = new PanelLayoutDto { Pages = { new PanelPageDto { Id = "p1", Widgets = { new PanelWidgetDto { Id = "w1", Type = "deck" } } } } },
        };

        DeckInstanceGc.Run(doc);

        Assert.True(doc.StreamDeck.Instances.ContainsKey("widget:w1"));
    }

    [Fact]
    public void WidgetInstance_StillInTheDashboardLayout_Survives()
    {
        var doc = new NexusSettings();
        doc.StreamDeck.Instances["widget:w1"] = new DeckInstance { Mode = "fixed" };
        doc.Panel.DashboardLayout = new PanelLayoutDto { Pages = { new PanelPageDto { Id = "p1", Widgets = { new PanelWidgetDto { Id = "w1", Type = "deck" } } } } };
    ...
    [Fact]
    public void StreamdeckInstance_NeverGarbageCollected()
    {
        var doc = new NexusSettings();
        doc.StreamDeck.Instances["streamdeck:SERIAL-1"] = new DeckInstance { Mode = "fixed" };

        DeckInstanceGc.Run(doc);

        Assert.True(doc.StreamDeck.Instances.ContainsKey("streamdeck:SERIAL-1"));
    }
}
